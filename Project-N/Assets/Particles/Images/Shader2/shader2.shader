		...
		SubShader
		{
			...
			Pass
			{
				...
				fixed4 frag(v2f i) : SV_Target
				{
					...
					i.uv.x = i.uv.x + noiseval * sin(_Time.y * 0.1f);
					i.uv.y = i.uv.y + noiseval * sin(_Time.y * 0.1f);

					int colid = 0;
					for (int h = 0; h < PALETTE_SIZE - 1; h++)
					{
						if (_ColourTimes[h] <= noiseval && _ColourTimes[h + 1] >= noiseval)
						{
							colid = h;
							break;
						}
					...
					//col = (noiseval, noiseval, noiseval, 1.0);
